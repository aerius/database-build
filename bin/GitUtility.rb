...
class GitUtility
  ...
  def self.authenticated_clone_url(repo_url)
    ...
    return repo_url.sub(%r{\Ahttps://}i, "https://#{uri_escape(username)}:#{uri_escape(token)}@")
  end

  # Copies a git working tree to target_dir, excluding gitignored files.
  # Includes tracked and untracked non-ignored files, plus .git for hash recording.
  def self.copy_working_tree_excluding_ignored(source_dir, target_dir)
    raise "Source not found: '#{source_dir}'" unless File.directory?(source_dir)
    raise "Source is not a git repository: '#{source_dir}'" if get_git_repo_root(source_dir).nil?

    entries = list_non_ignored_files(source_dir)
    FileUtils.rm_rf(target_dir) if File.exist?(target_dir)
    FileUtils.mkdir_p(target_dir)

    entries.each do |rel|
      src = File.join(source_dir, rel)
      # Only copy files or symlinks.
      next unless File.file?(src) || File.symlink?(src)
      dst = File.join(target_dir, rel)
      FileUtils.mkdir_p(File.dirname(dst))
      # preserve: true keeps mtimes for source_newer_than_target? skip/re-copy decisions.
      FileUtils.cp(src, dst, preserve: true)
    end

    # .git is not listed by ls-files; copy it so hash / uncommitted checks work on the target.
    git_dir = File.join(source_dir, '.git')
    if File.exist?(git_dir) then
      FileUtils.cp_r(git_dir, File.join(target_dir, '.git'))
    end
  end
  ...
  # Returns relative paths of tracked and untracked non-ignored files under path.
  # Used when copying a sibling checkout for a dev build (skip gitignored junk).
  # git ls-files flags: -z NUL-separated (safe for spaces); --cached tracked;
  # --others untracked; --exclude-standard apply .gitignore / exclude rules.
  def self.list_non_ignored_files(path)
    dir = File.directory?(path) ? path : File.dirname(path)
    curr_dir = Dir.pwd
    Dir.chdir(dir)
    cmd = "#{git_exe} ls-files -z --cached --others --exclude-standard #{git_stderr_null}"
    socket = IO.popen(cmd)
    begin
      raw = socket.gets(nil).to_s
    ensure
      socket.close
      Dir.chdir(curr_dir)
    end
    raise "git ls-files failed in '#{dir}'" unless $?.nil? || $? == 0
    return raw.split("\0").reject(&:empty?)
  end

  # Percent-encode for URL userinfo (tokens may contain non-alphanumeric characters).
  def self.uri_escape(value)
    return value.to_s.gsub(/([^a-zA-Z0-9\-_.~]+)/) {
      '%' + $1.unpack('H2' * $1.bytesize).join('%').upcase
    }
  end

  private_class_method :git_exe, :git_stderr_null, :run_git, :list_non_ignored_files, :authenticated_clone_url, :uri_escape
end
